...
import { initStatsUI } from './uiHandler.js';
...
export class ParticleSim {
  ...
  handleMerges() {
    ...
    this.groups.forEach((grpA, giA) => {
      ...
      if (grpA.depth >= grpA.maxDepth){ return};
      const posA = this.positions[giA];
      const mergeDist2 = this.groups.map(grpB =>
        (grpA.particleRadius + grpB.particleRadius) ** 1.1 + 1
      );
      ...
      for (let i = 0; i < grpA.amount; i++) {
        ...
        for (let giB = 0; giB < this.groups.length; giB++) {
          ...
          if (giA === giB || grpB.depth >= grpB.maxDepth) continue;
